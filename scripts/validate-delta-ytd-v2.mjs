...
function parseOrcamento(sheet) {
  ...
  for (let i=3;i<rows.length;i++){
    ...
    out.set(currentN4+"|"+nomeLB, { meses2026, total2026: num(row[14])??0 });
  }
  return out;
}
function parseRealizado(sheet) {
  const rows = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: null });
  const map = new Map(); let curAno=null, curN4=null;
  for (let i=1;i<rows.length;i++){
    ...
    const nome=row[3]; if(!nome||nome==="Total"||!curN4) continue;
    const key = curN4+"|"+nome;
    let a = map.get(key);
    if(!a){a={orc2026:0,real2026:0,emp2026:0}; map.set(key,a);}
...
const MES_ATUAL = 7;
let planejadoAcum = 0, executadoAcum = 0;
for (const k of allKeys) {
  const o = orc.get(k); const r = real.get(k);
  if (o) planejadoAcum += o.meses2026.slice(0, MES_ATUAL).reduce((a,b)=>a+b,0);
  else if (r) planejadoAcum += r.orc2026 * (MES_ATUAL/12);
  if (r) executadoAcum += r.real2026 + r.emp2026;
}
const delta = executadoAcum - planejadoAcum;
console.log("Planejado Acumulado (jan-jul):", planejadoAcum.toFixed(2));
console.log("Executado Acumulado (Realizado+EmPgto):", executadoAcum.toFixed(2));
console.log("Delta YTD (Executado - Planejado):", delta.toFixed(2), delta < 0 ? "(atrás do plano)" : "(à frente do plano)");
